...
pub fn get_genesis_multiaddr(
    ...
    if failed_vms.is_empty() {
        ...
        return Ok((successful, genesis_inventory));
    }
    ...
    if !failed_vms.is_empty() {
        ...
        return Err(Error::GenesisListenAddress(failed_ips));
    }

    println!("Successfully retrieved genesis multiaddrs: {all_addrs:?}",);
    Ok((all_addrs, genesis_inventory))
}
        ...
        match ssh_client.run_command(&genesis_ip, "root", &any_node_command, false) {
            ...
            Err(err) => {
                if attempt < MAX_RETRIES {
                    ...
                    return Err(Error::GenesisListenAddress(vec![genesis_ip]));
                }
            }
        }
    }

    Err(Error::GenesisListenAddress(vec![genesis_ip]))
}
